...
from Huber_class import SimSet1
from coreset import huber_coreset, huber_query_h
import time
    ...
    for epsilon_range in Epsilon_range:
        for sample_size_index in Sample_size_index:
            ...
            if not os.path.exists(path):
                os.makedirs(path)
            path_res = 'huber_energy_r_{}_xsigma_{}_y_sigma{}_samp_{}.mat'.format(
                epsilon_range,
            ...
            for c1 in range(1):
                for c2 in range(1):
                    ...
                    X_tr, y_tr, X_ts, y_ts = load_energy()
                    d = X_tr.shape[1]
                    ...
                    for t in range(1):
                        sim1 = SimSet1(X_tr.shape[1], N, epsilon_range, X_tr, y_tr, X_ts, y_ts, 1,
                                       np.ones((X_tr.shape[0], 1)), set_delta)
                        ...
                        print("1 Time taken in initial solve of model with N={0}: {1:.4f} s".format(
                            N, sim1.sol_time[0]), ' hl_ts: ', test_loss, 'l2_ts: ', test_predict)
                        sim1.M.dispose()
                        del (sim1)
                        gc.collect()
                    ...
                    for t in range(50):
                        ...
                        sim1 = SimSet1(X_tr_sample.shape[1], N, epsilon_range, X_tr_sample, y_tr_sample,
                                       X_ts,
                                       y_ts, 1, np.ones((X_tr_sample.shape[0], 1)) * N / Sample_size, set_delta)
                        ...
                        start = time.time()
                        X_coreset, y_coreset, weight = huber_coreset(X_tr, y_tr, coreset_size, wts3[0], b_val, set_delta,
                                                                  huber_query_h)
                        T += time.time() - start
                        print('5 ', X_coreset.shape[0])
                        sim1 = SimSet1(X_coreset.shape[1], N, epsilon_range, X_coreset, y_coreset, X_ts,
                                       y_ts, 1, weight, set_delta)
                        ...
                        print("Time taken in initial solve of model with N={0:.4f}: {1:.4f} s".format(
                            T, sim1.sol_time[0]), ' hl_ts: ', test_loss, 'l2_ts: ', test_predict)
